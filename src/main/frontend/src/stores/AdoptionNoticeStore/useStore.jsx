...
export function useStore() {
  ...
  const actions = {
    // 게시물 관련
    getNotices: () => dispatch(getNotices()),  // 게시물 목록 불러오기
    addNotice: (noticeData) => dispatch(addNotice(noticeData)),  // 게시물 추가
    updateNotice: (announcementNum, updatedData) => dispatch(updateNoticeAction(announcementNum, updatedData)),  // 게시물 수정
    deleteNotice: (announcementNum) => dispatch(deleteNoticeAction(announcementNum)),  // 게시물 삭제
    getNoticeByUid: (announcementNum) => dispatch(getNoticeByUid(announcementNum)),  // 게시물 상세 보기
