    ...
    switch (action.type) {
      ...
      case UPDATE_NOTICE:
        return {
          ...state,
          notices: state.notices.map((notice) =>
            notice.announcementNum === action.payload.announcementNum ? action.payload : notice
          ),
        };
      ...
      case DELETE_NOTICE:
        return {
          ...state,
          notices: state.notices.filter((notice) => notice.announcementNum !== action.payload),
        };
  
      // 특정 게시물 상세 보기 (announcementNum 사용)
      case FETCH_NOTICE_BY_UID:
