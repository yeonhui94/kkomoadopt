...
// 게시물 수정
export const updateNoticeAction = (announcementNum, updatedData) => async (dispatch) => {
  try {
    const response = await updateNotice(announcementNum, updatedData);
...
// 게시물 삭제
export const deleteNoticeAction = (announcementNum) => async (dispatch) => {
  try {
    await deleteNotice(announcementNum);
    dispatch({ type: DELETE_NOTICE, payload: announcementNum });
  } catch (error) {
    dispatch({ type: SET_ERROR, payload: error.message });
  }
};
...
// 게시물 상세 조회
export const getNoticeByUid = (announcementNum) => async (dispatch) => {
  try {
    const response = await fetchNoticeByUid(announcementNum);
