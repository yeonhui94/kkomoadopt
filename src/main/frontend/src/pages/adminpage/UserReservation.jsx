...
function UserReservation({ gridArea }) {
  ...
  const [selectedSubCategory, setSelectedSubCategory] = useState(""); // 서브 카테고리 필터 상태
  const [filteredData, setFilteredData] = useState([]); // 필터링된 데이터 상태

  const [allPosts, setAllPosts] = useState([ // 모든 게시물 상태로 관리
    { id: 1, category: "온라인문의", nickname: "재벌3세", title: "입양 관련해서 문의 드립니다.", date: "2024-11-28", number: "010-1234-1234", status: "답변 완료" },
    ...
    { id: 6, category: "온라인문의", nickname: "주근익불주먹", title: "봉사 관련해서 문의 드립니다.", date: "2024-11-23", number: "010-1234-1234", status: "답변 완료" },
  ]);
  ...
  // 답변을 제출하면 해당 상태를 "답변 완료"로 변경
  const handleAnswerSubmit = (postId) => {
    setAllPosts((prevPosts) =>
      prevPosts.map((post) =>
        post.id === postId ? { ...post, status: "답변 완료" } : post
      )
    );
  };
